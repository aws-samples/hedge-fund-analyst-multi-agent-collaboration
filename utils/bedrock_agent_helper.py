...
# Initialize Weave
weave.init("hedge-fund-multi-agent-collaboration")  
...
class AgentsForAmazonBedrock:
    ...
    def create_lambda(
        ...
        _lambda_function = self._lambda_client.create_function(
            ...
            Layers=lambda_layers if lambda_layers else []
        )
    ...
    def create_supervisor_agent(
        ...
        if kb_arn is not None:
            ...
            _supervisor_role_arn = self._create_agent_role(
                supervisor_agent_name, model_id, [kb_arn]
            )
        else:
            _supervisor_role_arn = self._create_agent_role(
                supervisor_agent_name, model_id
            )
    ...
    @weave.op(call_display_name='hedge-fund-multi-agent-logs')
    def invoke(
            self,
            input_text: str,
            agent_id: str,
